...
module FacebookAds
  ...
  class User < AdObject
    ...
    has_edge :ad_export_presets do |edge|
      edge.get 'AdExportPreset'
    end
    ...
    has_edge :admined_groups do |edge|
      edge.get 'Group'
    end
    ...
    has_edge :adnetworkanalytics do |edge|
      edge.get 'InsightsQueryResult' do |api|
        ...
        api.has_param :app_ids, { list: 'int' }
    ...
    has_edge :atlas_notifications do |edge|
      edge.get 'AtlasNotification' do |api|
    ...
    has_edge :blind_pigs do |edge|
      edge.get 'BlindPig'
    end

    has_edge :blocked do |edge|
      edge.get 'BlockedUserInfo'
    end
    ...
    has_edge :bulkcontacts do |edge|
      edge.delete do |api|
        api.has_param :contact_surface, { enum: %w{ORIGINAL MESSENGER CONNECTIONS CONTACTSAPP GROWTH_CONTACT_IMPORTER }}
      end
    end
    ...
    has_edge :companies do |edge|
      edge.get 'AtlasCompany'
    end

    has_edge :contacts_photos do |edge|
      edge.post do |api|
    ...
    has_edge :contactsmessengersync do |edge|
      edge.get 'ContactsMessengerSyncConfig'
    end
    ...
    has_edge :fblite_to_nt_transitions do |edge|
      edge.get 'FbLiteToNtTransitions'
    end
    ...
    has_edge :game_items do |edge|
      edge.get 'GameItem' do |api|
        api.has_param :item_id, 'object'
        api.has_param :status, { enum: -> { GameItem::STATUS }}
    ...
    has_edge :game_times do |edge|
      edge.get 'GameTime'
      edge.post 'GameTime' do |api|
        api.has_param :action, { enum: -> { GameTime::ACTION }}
      end
    end
    ...
    has_edge :hashed_friends do |edge|
      edge.get 'HashedUser'
    end
    ...
    has_edge :instagram_businesses do |edge|
      edge.get 'InstagramBusiness'
    end

    has_edge :interests do |edge|
      edge.get 'Page' do |api|
        api.has_param :target_id, 'string'
      end
    end
    ...
    has_edge :managed_groups do |edge|
      edge.get 'Group'
    end

    has_edge :managers do |edge|
      edge.get 'User'
    end

    has_edge :megaphone_top_stories do |edge|
      edge.get 'NativeMegaphone' do |api|
        api.has_param :zero_campaign, 'string'
        api.has_param :megaphone_location, { enum: -> { NativeMegaphone::MEGAPHONE_LOCATION }}
      end
    end

    has_edge :megaphones do |edge|
      edge.get 'MobileAppAlert'
    end
    ...
    has_edge :mobile_configs do |edge|
      edge.get 'UserMobileConfig' do |api|
        api.has_param :config_sections, { list: 'string' }
      end
    end
    ...
    has_edge :music_preferences do |edge|
      edge.get 'Page'
    end
    ...
    has_edge :notification_seen_states do |edge|
      edge.get 'UserNotificationSeenStateData' do |api|
    ...
    has_edge :nux_statuses do |edge|
      edge.get 'UserNuxStatuses' do |api|
        api.has_param :nux_ids, { list: 'string' }
      end
    end
    ...
    has_edge :page_status_cards do |edge|
      edge.get 'PageStatusCard'
    end
    ...
    has_edge :persistent_stream_profiles do |edge|
      edge.get 'PersistentStreamProfile'
    end
    ...
    has_edge :personal_ads_persona do |edge|
      edge.get 'PersonalAdsPersona'
    end

    has_edge :phones do |edge|
      edge.get 'ScimCompanyUserPhoneNumber'
    end
    ...
    has_edge :product_catalogs do |edge|
      edge.get 'ProductCatalog' do |api|
        api.has_param :owner_types, { list: { enum: -> { ProductCatalog::OWNER_TYPES }} }
        api.has_param :business_catalogs_with_role_filter, 'bool'
        api.has_param :name_filter, 'string'
        api.has_param :exclude_user_owned_catalog_segments_filter, 'bool'
      end
    end
    ...
    has_edge :promotable_tours do |edge|
      edge.get 'EventTour' do |api|
        api.has_param :page_id, 'object'
        api.has_param :include_past_tours, 'bool'
      end
    end
    ...
    has_edge :screennames do |edge|
      edge.delete do |api|
        api.has_param :service_type, { enum: %w{AIM GADU ICQ GTALK MSN SKYPE YAHOO YAHOO_JP QQ NATEON TWITTER HYVES ORKUT MYSPACE GROUPWISE CYWORLD MIXI QIP REDIFF_BOL VKONTAKTE EBUDDY MAILRU JABBER ICLOUD BBM BBM_PPID INSTAGRAM LINE WECHAT KAKAOTALK OTHERS SNAPCHAT TUMBLR SOUND_CLOUD LINKED_IN PINTEREST YOU_TUBE MEDIUM FOURSQUARE SPOTIFY VIMEO KIK ASK_FM OK GITHUB TWITCH WHATSAPP }}
    ...
    has_edge :seen_nuxes do |edge|
      edge.get 'Application' do |api|
        api.has_param :application, 'string'
      end
    end
    ...
    has_edge :trust_metrics do |edge|
      edge.get 'TrustMetrics' do |api|
        api.has_param :region, 'string'
      end
    end

    has_edge :vaultdeletedimages do |edge|
      edge.get 'VaultDeletedImage'
    end

    has_edge :vaultdevices do |edge|
      edge.get 'VaultDevice' do |api|
        api.has_param :identifier_value, 'string'
      end
    end

    has_edge :vaultimages do |edge|
      edge.get 'VaultImage'
    end
