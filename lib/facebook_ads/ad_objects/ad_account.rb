...
module FacebookAds
  ...
  class AdAccount < AdObject
    ...
    has_edge :activity_logs do |edge|
      edge.get 'AdAccountActivity' do |api|
        api.has_param :changed_all, { list: { enum: -> { AdAccountActivity::CHANGED_ALL }} }
        api.has_param :changed_any, { list: { enum: -> { AdAccountActivity::CHANGED_ANY }} }
    ...
    has_edge :ad_column_sizes do |edge|
      edge.get 'AdColumnSizes'
    end

    has_edge :ad_custom_derived_metrics do |edge|
      edge.get 'AdCustomDerivedMetrics'
    end
    ...
    has_edge :ad_report_builder_reports do |edge|
      edge.get 'AdsReportBuilderSavedReport'
    end
    ...
    has_edge :adcampaignconversions do |edge|
      edge.get 'AdConversionValues' do |api|
        api.has_param :campaign_ids, { list: 'string' }
        api.has_param :include_deleted, 'bool'
        api.has_param :start_time, 'datetime'
        api.has_param :end_time, 'datetime'
        api.has_param :time_start, 'datetime'
        api.has_param :time_stop, 'datetime'
        api.has_param :aggregate_days, 'int'
        api.has_param :by_impression_time, 'bool'
        api.has_param :offset, 'int'
      end
    end

    has_edge :adcampaignstats do |edge|
      edge.get 'AdCampaignStats' do |api|
        api.has_param :start_time, 'datetime'
        api.has_param :end_time, 'datetime'
        api.has_param :time_start, 'datetime'
        api.has_param :time_stop, 'datetime'
        api.has_param :action_attribution_days_after_click, { enum: -> { AdCampaignStats::ACTION_ATTRIBUTION_DAYS_AFTER_CLICK }}
        api.has_param :action_attribution_days_after_imp, { enum: -> { AdCampaignStats::ACTION_ATTRIBUTION_DAYS_AFTER_IMP }}
        api.has_param :include_deleted, 'bool'
      end
    end
    ...
    has_edge :addrafts do |edge|
      edge.get 'AdDraft' do |api|
        api.has_param :author_id, 'int'
      end
    end

    has_edge :adgroupconversions do |edge|
      edge.get 'AdConversionValues' do |api|
        api.has_param :start_time, 'datetime'
        api.has_param :end_time, 'datetime'
        api.has_param :time_start, 'datetime'
    ...
    has_edge :adgroupstats do |edge|
      edge.get 'AdCampaignStats' do |api|
        ...
        api.has_param :campaign_ids, { list: 'string' }
        api.has_param :offset, 'int'
        api.has_param :stats_mode, 'string'
        api.has_param :include_deleted, 'bool'
      end
    end
    ...
    has_edge :adscript_execution_histories do |edge|
      edge.get 'AdScriptExecutionHistory'
    end

    has_edge :adscripts do |edge|
      edge.get 'AdScript'
    end
    ...
    has_edge :adspaymentcycle do |edge|
      edge.get 'AdsPaymentCycle'
    end
    ...
    has_edge :async_sessions do |edge|
      edge.get 'AsyncSession'
    end
    ...
    has_edge :audience_insights_study_rules do |edge|
      edge.get 'AudienceInsightsRule' do |api|
        api.has_param :partial_name, 'string'
        api.has_param :exact_name, 'string'
      end
    end

    has_edge :audience_studies do |edge|
      edge.get 'AudienceStudy' do |api|
        api.has_param :fields, { list: 'string' }
        api.has_param :study_status, { enum: -> { AudienceStudy::STUDY_STATUS }}
        api.has_param :name, 'string'
        api.has_param :description, 'string'
        api.has_param :author_id, 'object'
      end
    end

    has_edge :audienceoverlap do |edge|
      edge.get 'AudienceOverlap' do |api|
        api.has_param :pivot_id, 'int'
        api.has_param :comparison_ids, { list: 'int' }
        api.has_param :sort_by_overlap, 'bool'
        api.has_param :limit, 'int'
      end
    end
    ...
    has_edge :audiencesharing_accountvalid do |edge|
      edge.get 'AudienceSharingAccountValid' do |api|
        api.has_param :recipient_adaccount, 'string'
      end
    end

    has_edge :audiencesharing_recipientaccounts do |edge|
      edge.get 'AudienceSharingRecipientAccounts'
    end
    ...
    has_edge :brandlift_studies do |edge|
      edge.get 'ResearchPollStudy'
    end

    has_edge :brazil_credit_memos do |edge|
      edge.get 'BrazilCreditMemo' do |api|
        api.has_param :time_start, 'object'
        api.has_param :time_stop, 'object'
      end
    end
    ...
    has_edge :business_ad_account_requests do |edge|
      edge.get 'Business'
    end
    ...
    has_edge :checkout_payments do |edge|
      edge.get 'AdAccountCheckoutPayment'
    end

    has_edge :column_suggestions do |edge|
      edge.get 'ColumnSuggestions'
    end
    ...
    has_edge :conversions do |edge|
      edge.get 'AdConversions' do |api|
        api.has_param :start_time, 'datetime'
        api.has_param :end_time, 'datetime'
        api.has_param :time_start, 'datetime'
        api.has_param :time_stop, 'datetime'
    ...
    has_edge :current_addrafts do |edge|
      edge.get 'AdDraft'
    end
    ...
    has_edge :delivery_insights_activities do |edge|
      edge.get 'AdActivity' do |api|
        api.has_param :after, 'string'
        api.has_param :limit, 'int'
        api.has_param :since, 'datetime'
        ...
        api.has_param :uid, 'int'
        api.has_param :business_id, 'string'
        api.has_param :ad_set_ids, { list: 'string' }
        api.has_param :add_children, 'bool'
        api.has_param :activity_type, { enum: -> { AdActivity::ACTIVITY_TYPE }}
      end
    end
    ...
    has_edge :dpa_eligible_product_catalogs do |edge|
      edge.get 'ProductCatalog'
    end

    has_edge :emailimport do |edge|
      edge.get 'EmailImport' do |api|
    ...
    has_edge :engagementinsightsrules do |edge|
      edge.get 'EngagementInsightsRule' do |api|
        api.has_param :rule_ids, { list: 'int' }
      end
    end

    has_edge :export_ads do |edge|
      edge.get 'ExportAdgroup' do |api|
        api.has_param :time_range, 'object'
      end
    end

    has_edge :export_adsets do |edge|
      edge.get 'ExportAdCampaign' do |api|
        api.has_param :time_range, 'object'
      end
    end

    has_edge :export_campaigns do |edge|
      edge.get 'ExportAdCampaignGroup' do |api|
        api.has_param :time_range, 'object'
      end
    end

    has_edge :fame_ads do |edge|
      edge.get 'FameAdGroup' do |api|
        api.has_param :date_preset, { enum: -> { FAMEAdGroup::DATE_PRESET }}
        api.has_param :effective_status, { list: 'string' }
        api.has_param :time_range, 'object'
        api.has_param :include_deleted, 'bool'
        api.has_param :updated_since, 'int'
    ...
    has_edge :fame_adsets do |edge|
      edge.get 'FameAdCampaign' do |api|
        api.has_param :date_preset, { enum: -> { FAMEAdCampaign::DATE_PRESET }}
        api.has_param :effective_status, { list: { enum: -> { FAMEAdCampaign::EFFECTIVE_STATUS }} }
        api.has_param :time_range, 'object'
        api.has_param :include_deleted, 'bool'
        api.has_param :is_completed, 'bool'
      end
    ...
    has_edge :fame_campaigns do |edge|
      edge.get 'FameAdCampaignGroup' do |api|
        api.has_param :date_preset, { enum: -> { FAMEAdCampaignGroup::DATE_PRESET }}
        api.has_param :effective_status, { list: { enum: -> { FAMEAdCampaignGroup::EFFECTIVE_STATUS }} }
        api.has_param :time_range, 'object'
      end
    end
    ...
    has_edge :kpi_shifts do |edge|
      edge.get 'AdKpiShift' do |api|
        api.has_param :ad_set_ids, { list: 'string' }
        api.has_param :event_source, { enum: -> { AdKpiShift::EVENT_SOURCE }}
        api.has_param :notif_id, 'string'
      end
    end
    ...
    has_edge :liable_addresses do |edge|
      edge.get 'CrmAddress'
    end
    ...
    has_edge :nota_fiscals do |edge|
      edge.get 'NotaFiscal' do |api|
        api.has_param :time_start, 'object'
        api.has_param :time_stop, 'object'
      end
    end
    ...
    has_edge :payment_details_list do |edge|
      edge.get 'AdAccountPaymentDetails' do |api|
        api.has_param :payment_ids, { list: 'string' }
        api.has_param :payment_status, 'string'
        api.has_param :should_filter_by_due_date_cc_id, 'bool'
      end
    end
    ...
    has_edge :placement_usage do |edge|
      edge.get 'AdAccountPlacementUsage'
    end
    ...
    has_edge :reporting do |edge|
      edge.get 'AdsReportBuilder' do |api|
        ...
        api.has_param :dimension_groups, { list: { list: 'adaccountreporting_dimension_groups_enum_param' } }
        ...
        api.has_param :offset, 'int'
        api.has_param :limit, 'int'
        api.has_param :pagination_key, 'string'
        api.has_param :last_dimension, 'object'
    ...
    has_edge :reportstatsmetadata do |edge|
      edge.get 'ReportStatsMetadata' do |api|
        api.has_param :data_columns, { list: 'string' }
        api.has_param :actions_group_by, { list: { enum: -> { ReportStatsMetadata::ACTIONS_GROUP_BY }} }
        api.has_param :time_ranges, 'object'
        api.has_param :time_increment, 'string'
        api.has_param :time_interval, 'object'
        api.has_param :date_preset, { enum: -> { ReportStatsMetadata::DATE_PRESET }}
        api.has_param :filters, { list: 'object' }
      end
    end
    ...
    has_edge :send_bill_to_addresses do |edge|
      edge.get 'CrmAddress'
    end

    has_edge :sold_to_addresses do |edge|
      edge.get 'CrmAddress'
    end

    has_edge :spendlimits do |edge|
      edge.get 'AdAccountSpendLimit' do |api|
        api.has_param :min_time, 'datetime'
      end
    end
    ...
    has_edge :study_audiences do |edge|
      edge.get 'CustomAudience' do |api|
        api.has_param :fields, { list: 'string' }
        api.has_param :study_status, { enum: -> { CustomAudience::STUDY_STATUS }}
        api.has_param :name, 'string'
        api.has_param :description, 'string'
        api.has_param :author_id, 'object'
      end
    ...
    has_edge :targetingsearchrecommendation do |edge|
      edge.get 'AdAccountTargetingUnified' do |api|
        api.has_param :q, 'string'
        api.has_param :limit_type, { enum: -> { AdAccountTargetingUnified::LIMIT_TYPE }}
        api.has_param :whitelisted_types, { list: { enum: -> { AdAccountTargetingUnified::WHITELISTED_TYPES }} }
    ...
    has_edge :targetingsearchreplacements do |edge|
      edge.get 'AdAccountTargetingUnified' do |api|
        api.has_param :targeting_option_id, 'string'
    ...
    has_edge :topic_discovery_responses do |edge|
      edge.get 'TopicDiscoveryResponse' do |api|
        api.has_param :query_terms, { list: 'string' }
        api.has_param :limit, 'int'
        api.has_param :query_method, { enum: -> { TopicDiscoveryResponse::QUERY_METHOD }}
    ...
    has_edge :transaction_details do |edge|
      edge.get 'TransactionDetails' do |api|
        api.has_param :transaction_keys, { list: 'string' }
      end
    end
    ...
    has_edge :unique_adcreatives do |edge|
      edge.get 'UniqueAdCreative' do |api|
        api.has_param :time_range, 'object'
        api.has_param :date_preset, { enum: -> { UniqueAdCreative::DATE_PRESET }}
        api.has_param :sort, { list: 'string' }
        api.has_param :filtering, { list: 'object' }
      end
    end
